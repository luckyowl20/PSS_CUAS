...
class RigApp(QWidget):
    ...
    def _on_analyze_click(self):
        ...
        self._update_cone_side_surface_via_max(
            theta_min=-90.0, theta_max=90.0, theta_step=1.0,
            n_side=24, phi_hi=90.0,
            ray_len=250.0, start_at="pivot",
            opacity=0.8,
            cap_samples_angle=181,   # was cap_pos/neg before; now one full YZ cap
    ...
    def _build_cone_side_surface_via_max(self,
                                        theta_min=-90.0, theta_max=90.0, theta_step=1.0,
                                        n_side=24, phi_hi=90.0,
                                        ray_len=180.0,
                                        start_at="pivot"):
        """
        Lateral surface only. Rays originate at the pivot.
        Bottom ring is the smooth intersection with the sphere |p - pivot| = l
        (l = current barrel length), so the surface starts exactly at the muzzle distance.
        """
        # Sample rim directions (θ, φ_max(θ))
        thetas = np.arange(theta_min, theta_max + 1e-9, theta_step, dtype=float)
        phi_max = np.array([self._max_phi_for_theta(float(th), phi_hi=phi_hi)
    ...
    def _build_full_yz_cap_via_max(self,
                                theta_pos=+90.0, theta_neg=-90.0,
                                n_angle=181,      # in-plane angle samples across −Y..+Y
                                n_radial=3,       # annular strips; small is fine (it’s planar)
                                phi_hi=90.0,
                                ray_len=180.0,
                                start_at="pivot"):
        """
        One planar cap in the YZ (x=0) plane that spans BOTH +Y and −Y.
        Inner edge is the smooth spherical cutoff |p - pivot| = l (same as side surface).
        """
        # Max φ on the two YZ edges
        phi_max_pos = self._max_phi_for_theta(theta_pos, phi_hi=phi_hi)
        phi_max_neg = self._max_phi_for_theta(theta_neg, phi_hi=phi_hi)
    ...
    def _update_cone_side_surface_via_max(self,
                                        theta_min=-90.0, theta_max=90.0, theta_step=1.0,
                                        n_side=24, phi_hi=90.0,
                                        ray_len=180.0, start_at="pivot",
                                        opacity=0.35,
                                        cap_samples_angle=181, cap_samples_radial=3, cap_opacity=0.35):
        # remove old
        if hasattr(self, "cone_surface_actor") and self.cone_surface_actor is not None:
            try: self.plotter.remove_actor(self.cone_surface_actor, reset_camera=False)
        ...
        pts, faces = self._build_cone_side_surface_via_max(
        ...
        if pts is None:
            self.plotter.render()
            self.status.setText("No feasible side surface in the requested θ window.")
            return
        mesh = pv.PolyData(pts, faces)
        self.cone_surface_actor = self.plotter.add_mesh(
        ...
        cap_pts, cap_faces = self._build_full_yz_cap_via_max(
        ...
        if cap_pts is not None:
            ...
            self.plotter.add_mesh(cap, color="red", opacity=cap_opacity,
